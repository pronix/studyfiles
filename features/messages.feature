...
@green
  Сценарий: Отправка сообщения пользователю
    ...
    И я введу в поле "Ваше сообщение" значение "Тестовое сообщение"
    И нажму "Отправить"
...
@green
  Сценарий: Переключение между дискуссиями
    Допустим есть пользователи:
      | email              | name            |
      | user10@example.com | Максим Петрович |
      | user11@example.com | Леонид Юрьевич  |
    ...
    Допустим пользователь "user11@example.com" написал пользователю "user@example.com" 1 сообщение
    И последнее сообщение от пользователя "user11@example.com" содержит текст "Проверка сообщения"
    И последнее сообщение от пользователя "user10@example.com" содержит текст "От второго меня"
    Допустим я авторизован как пользователь с логином и паролем "user@example.com/secret"
    И я иду на страницу дискуссий
    И я перейду по ссылке "Леонид Юрьевич"
    И я должен увидеть "Проверка сообщения"
    И я перейду по ссылке "Максим Петрович"
    И я не должен увидеть "Проверка сообщения"
    И я должен увидеть "От второго меня"
# @green    
#   Сценарий: Я не должен видеть созданные дискуссии без сообщени
#     Допустим есть пользователи:
#       | email              | name            |
#       | user10@example.com | Максим Петрович |
#       | user11@example.com | Леонид Юрьевич  |
#     Допустим пользователь "user10@example.com" написал пользователю "user@example.com" 1 сообщение
#     И пользователь "user11@example.com" просто имеет со мной дискуссию
#     Допустим я авторизован как пользователь с логином и паролем "user@example.com/secret"
#     И я иду на страницу дискуссий
#     И я не должен видеть "Леонид Юрьевич" в собеседниках

@green
  Сценарий: Пометка о прочетнии
    Допустим есть пользователи:
      | email              | name            |
      | user10@example.com | Максим Петрович |
      | user11@example.com | Леонид Юрьевич  |
