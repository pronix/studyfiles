...
#В архиве "test.zip" находятся 10 файлов. sha архива = "009d5b586b0913e1e9f416dbc739a180fd8aa4af"
  Сценарий:
    Допустим я авторизован как пользователь с логином и паролем "user@example.com/secret"
    И я перейду на страницу Мои файлы
    И я перейду по ссылке "загрузить файлы"
    И я прикреплю файл "test.zip" в поле "document_item"
    И нажму "Загрузить"
#    Тогда в таблице delayed_jobs должна появится новая запись
    И я должен видеть "Ваш архив принят на обработку"
    Тогда delayed_job подхыватывает задачу
    И по окончанию работы в табилце документов должно появится "10" записей
    И файл "9d/00/009d5b586b0913e1e9f416dbc739a180fd8aa4af" должен быть удален.
#    И в таблице delayed_jobs не должно быть записей